...
export default function CreatorListingEditorPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const [{ user }, listing] = await Promise.all([getCurrentProfile(client), fetchListing(client, params.id)]);
